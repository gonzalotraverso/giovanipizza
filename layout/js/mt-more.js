...
// Load this file's selection again by visiting: http://mootools.net/more/a45332771f82e69e898a51e287d11eea 
...
Form.Upload = new Class({
    ...
    if(input.hasClass('FormUploadActive'))
      return;
    input.addClass('FormUploadActive');
		this.setOptions(options);
	...
	modernUpload: function(input){
		...
		var drop = this.drop = new Element('div.droppable', {
			html: "<span>"+this.options.dropMsg+"</span>"
		}).inject(input, 'after');
    
		var list = new Element('ul.uploadList').inject(drop, 'after');

		var progresscont = new Element('div.progresscont').inject(list, 'after');
		var progress = this.progress = new Element('div.progress')
			.setStyle('display', 'none').inject(progresscont)
    
		var uploadReq = new Request.File({
			url: form.get('action'),
			onRequest: progress.setStyles.pass({display: 'block', width: 0}, progress),
			onProgress: function(event){
				var loaded = event.loaded, total = event.total, percent = parseInt(loaded / total * 100, 10) + '%';
				progress.setStyle('width', percent);
				drop.set('html', percent);
			},
			onComplete: function(){
				progress.tween('opacity', 0);
        drop.set('html', '');
		...
		var  inputFiles = new Form.MultipleFileInput(input, list, document.body, {
			onDragover: drop.addClass.pass('hover', drop),
    ...
    this.drop.set('html', percent);
    this.drop.addClass('uploading');
		inputFiles.getFiles().each(function(file){
			uploadReq.append(inputname , file);
		});
    Object.each(this.options.data, function(key, value){
      uploadReq.append(value, key);
    });
		uploadReq.send();
    this.reset();
	},
...
Form.MultipleFileInput = new Class({
	...
	initialize: function(input, list, drop, options){
		...
		this.dragEvents = drop && (typeof document.body.draggable != 'undefined') ? {
			...
			drop: function(event){
				event.preventDefault();
        if(this.drop.hasClass('uploading'))
          return;
        this.drop.addClass('uploading');
    ...
    this.drop.removeClass('uploading');
		this.attach();
	},
